...
| `rndtime` | Modifier | `\dur, Pwhite(lo, hi)` | `[N]` |
...
| `mm_inline` | Modifier | `\stretch, 60/BPM` | `[BPM]` (from `\|\|N\|\|`) |
| `sustainstart_` | Modifier | `\sustain, 1` | `[]` |
| `sustainstop_` | Modifier | `\sustain, 0` | `[]` |
| `sustainstopstart_` | Modifier | `\sustain, 1` | `[]` |
| `sostenutostart_` | Modifier | `\sostenuto, 1` | `[]` |
| `sostenutostop_` | Modifier | `\sostenuto, 0` | `[]` |
| `softstart_` | Modifier | `\softPedal, 1` | `[]` |
| `softstop_` | Modifier | `\softPedal, 0` | `[]` |
| `legato_` | Modifier | `\legato, 1.5` | `[]` |
| `nolegato_` | Modifier | `\legato, 0.8` | `[]` |
...
Represents tied notes (legato connection between notes). Generated by the
MusicXML importer when notes are connected via ties in the score.
...
**Parsing:** The parser recognizes:
- `C4&`, `fa4&`, `C#4&` — tie start (note should sustain beyond its duration)
- `&C4`, `&fa4`, `&C#4` — tie end (continuation of previously tied note)

Both French solfege and Anglo note formats are supported.

**Emission:** Tie starts emit with `\legato, 2.0` (extended sustain). Tie ends
that match a pending tie start emit as `Event.silent(0.25)` since the note
is already being held.

**BP3:** `C4&` -> `Tie(note=Note(name="C", octave=4), is_start=True)`
**BP3:** `&C4` -> `Tie(note=Note(name="C", octave=4), is_start=False)`
**BP3:** `fa4&` -> `Tie(note=Note(name="fa", octave=4), is_start=True)`
**BP3:** `&fa4` -> `Tie(note=Note(name="fa", octave=4), is_start=False)`
